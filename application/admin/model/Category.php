...
{	
    ...
    */
	public function getTrees($array = [], $fieldArr = ['cate_name', 'cate_id'], $pid = 0, $level = 0, $isStr = false)
	{
		...
		foreach ($array as $key => $value) {
			...
			if ($value['pid'] == $pid)
			{
				...
				$this->getTrees($array, $fieldArr, $value[$fieldArr[1]], $level+1, $isStr);
			}
	...
	public function arrayMoreToOne(&$array = [], $fieldArr = ['cate_name', 'cate_id'], $str = 'son', $level = 0 , $isStr = true)
	{
		...
		foreach ($array as $key => $value) {
			...
			if (!empty($array[$key][$str]))
			{
				...
				$this->arrayMoreToOne($value[$str], $fieldArr, $str, $level + 1 , $isStr);
			} else {
				if ($level > 0 && $isStr == true)
					$array[$key][$fieldArr[0]] = '|' . str_repeat('----', $level) . $value[$fieldArr[0]];
				$list[] = $array[$key];
	 ...
	 * @return [type]          [description]
	 */
	public function arrayMoreToFore($array = [], $fieldArr = ['cate_name', 'cate_id'], $pid = 0, $level = 0, $isStr = true, $num = 0)
	{
		//第一步 构造数据
		static $list = [''];
		foreach ($array as $key => $value) {
			...
			if ($value['pid'] == $pid)
			{
				...
				unset($array[$key]);
				//开始递归,查找父ID为该节点ID的节点,级别则为原级别+1
				$this->arrayMoreToFore($array, $fieldArr, $value[$fieldArr[1]], $level+1, $isStr, $num);
			}
